...
//
    ...
    //
        ...
        system.exec('npm test ' + name.substr(1,name.length-1), function(code, output) {
            if (output.match(/npm ERR/i)) {
    ...
    //
        ...
        fs.writeFileSync('./runners/'+name.substr(1,name.length)+'.js',"");

    //
    // [&] Blah Add Package To package.json (like npm --save) ([name])
    //
    } else if (name[0]=="&") {
        saveToPackage = true;
                ...
                try {
                    ...
                    try {
                        ...
                        if (~(name + "").indexOf('./')) {
                            ...
                            if (saveToPackage) {
                                system.exec('npm install ' + name + ' --save', function(code, output) {
                                    if (output.match(/npm ERR/i)) {
